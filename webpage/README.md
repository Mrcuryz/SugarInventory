...
- 每条消息都应展示轻量身份信息。助手侧名称固定为“智能仓储助手”，普通对话界面不展示模型名称；后端仍可在会话数据中保留 `modelDisplayName` 供后续调试或多模型路由扩展。
- 如果后端未返回模型名称，不应在普通对话界面误报为配置错误；模型可观测性应通过后端日志、审计或管理员调试能力补齐。
- 用户侧名称直接取当前登录态的用户姓名或员工标识，不允许让前端用户手动输入或伪造身份。
- 抽屉顶部不再放独立会话身份面板，避免挤占对话高度；“会话已连接”等状态移入标题栏，标题栏不展示模型名称和调试入口。
- 头像和名称应延续当前 Web 管理端的浅色、低噪声、业务工具风格，不使用营销式大头像或高饱和装饰。
- 输入框交互遵循常见对话产品习惯：Enter 直接发送，Shift+Enter 换行；中文输入法组合输入期间不得误触发送。

P4 会话挂起体验要求：

- 点击抽屉遮罩、右上角关闭图标或业务跳转时，只收起 AI 助手抽屉，不撤销 Agent 会话，不清空消息列表和待选择状态。
- 用户再次点击“AI 助手”时，复用当前前端持有的 Agent 会话和对话上下文，避免临时处理业务后丢失“这些”“它”“刚才那个”等上下文。
- 只有点击“结束会话”并确认后，前端才调用 `DELETE /api/agent/sessions/{agentSessionId}` 撤销会话，并清理本次前端对话状态。
