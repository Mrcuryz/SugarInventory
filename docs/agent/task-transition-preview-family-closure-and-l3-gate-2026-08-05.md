...
既有人工成品入库服务已完成第一轮代码加固：批量入口限制最多 20 条；整批在任何业务写入前完成托盘码规范化、不同标签同托盘去重和实体解析；确认与取消统一按托盘主键排序并使用显式行锁；标签关联在预解析后变化时失败关闭。新增单元合同覆盖上述边界和整批事务注解边界；真实第二项失败回滚仍留给隔离数据库复验，未用 Mockito 结果代替。

该结果把“确认/取消锁顺序”和“批量上限、去重、排序”推进为代码级 `GO`，但没有开放 Agent 写入。真实 MySQL 锁等待下的并发胜者和整批事务回滚尚未使用隔离样本复验，因此业务加固总体仍为 `PARTIAL`，L3 继续 `NO-GO`。
