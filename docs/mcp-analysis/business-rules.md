...
- 创建产品时校验产品名称唯一；更新时校验枚举合法性并支持动态更新。产品存在当前库存时，禁止修改 `pieces_per_pallet` 和 `weight_per_piece`；不同规格应新建产品。
...
- 当前库存实行一板一码；不足满板也独占一个二维码板位。此时 `quantity=1` 表示板位占用，`pieces` 表示该板实际件数，统计时不得再叠加一整板。
...
- 业务已确认：`assay.sample_date` 表示生产日期，`product_id + sample_date` 是产品批次键。
- 当前批次化验必须按产品和生产日期选择最新版本，排序为 `version DESC, created_at DESC, id DESC`；库存、托盘和任务上的 `assay_id` 仅可用于历史追溯、兼容或诊断，不能作为当前质量状态的唯一事实来源。
- 产品允许先入库、后补化验；化验补录或新增版本后，当前只读查询应自动读取最新版本，不要求先同步库存外键。
- “符合某个标准”默认读取化验生成时保存的判定结果和标准快照；“按当前标准重新判断历史化验”属于单独的 L2 预览能力。
- “原始指标满足数值条件”读取当前批次最新化验的原始数值；指标字段、运算符和条件数量必须由后端白名单约束，LLM 不生成 SQL，也不替代后端作合格判定。

待确认：

- 多个候选标准“首个”的排序依据需要检查 Mapper 的 `selectActiveByProductId` SQL。
- `is_qualified` 数据库旧枚举只有 `合格/不合格`，但代码可能写入 `无标准`；需确认迁移是否已放宽枚举。
- 业务确认上线时清除旧库存，目标当前库存实行一板一码；批次生产日期只取 `pallet_code.production_date`，不再以 `inventory.entry_date` 回退。仍需在上线前禁用或约束可产生非托盘库存的旧写入路径，详见 `docs/agent/inventory-assay-batch-semantics.md`。
- `qualified_standards` 主要保存名称，不能稳定区分同名标准的不同版本；版本级标准命中查询需要补充结构化命中标准快照。
