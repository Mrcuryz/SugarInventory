...
本机标签打印助手、浏览器 `localhost` 接口和打印机控制不属于 MCP Tool。服务端 Agent 不得发现、代理或调用这些端点；二维码打印状态查询工具只查询服务端业务事实，不获得本机打印权。
...
2026-08-10 起，`query_fixed_product_qr_pool` 也可作为成品入库固定二维码路径的 L1 支撑工具，但不改变工具风险等级或写入边界。Runtime 必须先确认具体产品、目标库位和二维码来源，再强制限定具体产品规范名与“仅空闲”；安全适配层继续过滤非精确产品、未启用固定模式和非空闲记录，并把内部状态转为用户语言。该查询不会创建任务或写库存。用户选择固定二维码后，由受控 Web 弹窗复用现有 `/api/pallet-codes/fixed-product/activate/pdf` 入口创建待入库任务；这仍不是 MCP Tool，也不能跳过后续任务资格预览、精确入库预览和用户确认。该动作会产生待处理任务和 `FINISH_BIND` 二维码生命周期追溯，但不会写入库存或库存移动账本。
