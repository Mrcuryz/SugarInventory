...
- checksum 与运行包完全一致；
- 基础业务表及迁移声明创建的表真实存在。
- 2026-08-13 起额外核对成品入库/智能报数关键列、旧引用可空约束和幂等唯一索引，避免出现“迁移账本全绿但运行字段或唯一约束缺失”。当前最小合同为 33 个关键列、1 个可空约束和 7 个关键索引；不把所有业务字段硬编码为启动门禁。
...
2026-08-13 当前本地 `laibin` 验收库按上述既有结构审计结论重新纳入受控迁移：截至 `2026-07-24-add-agent-review-view-permission.sql` 的 29 条登记为 `BASELINED`，其后 13 条真实执行为 `APPLIED`，最终 42/42。开启新 schema 门禁后真实启动通过：48 张必需表、33 个关键列、1 个可空约束和 7 个关键索引就绪。该记录仍不代表生产数据库已迁移。
