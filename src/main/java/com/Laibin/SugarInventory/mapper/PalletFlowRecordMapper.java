...
public interface PalletFlowRecordMapper extends BaseMapper<PalletFlowRecord> {
    ...
    @Select({
            ...
            " CASE",
            "   WHEN fr.from_warehouse_id IS NOT NULL THEN fr.from_warehouse_id",
            "   WHEN fr.operation_type IN ('OUT', 'PREPARE_CONSUMED', 'ORDER_MATERIAL_PICK') THEN ci.warehouse_id",
            "   ELSE fr.from_warehouse_id",
            " END AS fromWarehouseId,",
            " fw.warehouse_name AS fromWarehouseName,",
            " CASE",
            "   WHEN fr.from_side IS NOT NULL THEN fr.from_side",
            "   WHEN fr.operation_type IN ('OUT', 'PREPARE_CONSUMED', 'ORDER_MATERIAL_PICK') THEN ci.side",
            "   ELSE fr.from_side",
            " END AS fromSide,",
            " CASE",
            "   WHEN fr.from_row_number IS NOT NULL THEN fr.from_row_number",
            "   WHEN fr.operation_type IN ('OUT', 'PREPARE_CONSUMED', 'ORDER_MATERIAL_PICK') THEN ci.`row_number`",
            "   ELSE fr.from_row_number",
            " END AS fromRowNumber,",
            " CASE",
            "   WHEN fr.from_layer IS NOT NULL THEN fr.from_layer",
            "   WHEN fr.operation_type IN ('OUT', 'PREPARE_CONSUMED', 'ORDER_MATERIAL_PICK') THEN ci.layer",
            "   ELSE fr.from_layer",
            " END AS fromLayer,",
            " CASE",
            "   WHEN fr.to_warehouse_id IS NOT NULL THEN fr.to_warehouse_id",
            "   WHEN fr.operation_type IN ('SEMI_INSTOCK', 'FINISH_INSTOCK', 'TRANSFER') THEN ci.warehouse_id",
            "   ELSE fr.to_warehouse_id",
            " END AS toWarehouseId,",
            " tw.warehouse_name AS toWarehouseName,",
            " CASE",
            "   WHEN fr.to_side IS NOT NULL THEN fr.to_side",
            "   WHEN fr.operation_type IN ('SEMI_INSTOCK', 'FINISH_INSTOCK', 'TRANSFER') THEN ci.side",
            "   ELSE fr.to_side",
            " END AS toSide,",
            " CASE",
            "   WHEN fr.to_row_number IS NOT NULL THEN fr.to_row_number",
            "   WHEN fr.operation_type IN ('SEMI_INSTOCK', 'FINISH_INSTOCK', 'TRANSFER') THEN ci.`row_number`",
            "   ELSE fr.to_row_number",
            " END AS toRowNumber,",
            " CASE",
            "   WHEN fr.to_layer IS NOT NULL THEN fr.to_layer",
            "   WHEN fr.operation_type IN ('SEMI_INSTOCK', 'FINISH_INSTOCK', 'TRANSFER') THEN ci.layer",
            "   ELSE fr.to_layer",
            " END AS toLayer,",
            " fr.remark AS remark,",
            ...
            " LEFT JOIN inventory ci ON ci.pallet_code_id = fr.pallet_code_id",
            " LEFT JOIN warehouse fw ON fw.id = CASE",
            "   WHEN fr.from_warehouse_id IS NOT NULL THEN fr.from_warehouse_id",
            "   WHEN fr.operation_type IN ('OUT', 'PREPARE_CONSUMED', 'ORDER_MATERIAL_PICK') THEN ci.warehouse_id",
            "   ELSE fr.from_warehouse_id",
            " END",
            " LEFT JOIN warehouse tw ON tw.id = CASE",
            "   WHEN fr.to_warehouse_id IS NOT NULL THEN fr.to_warehouse_id",
            "   WHEN fr.operation_type IN ('SEMI_INSTOCK', 'FINISH_INSTOCK', 'TRANSFER') THEN ci.warehouse_id",
            "   ELSE fr.to_warehouse_id",
            " END",
