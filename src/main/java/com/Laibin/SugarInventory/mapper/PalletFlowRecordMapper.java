...
public interface PalletFlowRecordMapper extends BaseMapper<PalletFlowRecord> {
    ...
    @Select({
            ...
            "   WHEN fr.operation_type IN ('OUT', 'PREPARE_CONSUMED') THEN ci.warehouse_id",
            ...
            "   WHEN fr.operation_type IN ('OUT', 'PREPARE_CONSUMED') THEN ci.side",
            "   ELSE fr.from_side",
            " END AS fromSide,",
            " CASE",
            "   WHEN fr.from_row_number IS NOT NULL THEN fr.from_row_number",
            "   WHEN fr.operation_type IN ('OUT', 'PREPARE_CONSUMED') THEN ci.`row_number`",
            "   ELSE fr.from_row_number",
            " END AS fromRowNumber,",
            " CASE",
            "   WHEN fr.from_layer IS NOT NULL THEN fr.from_layer",
            "   WHEN fr.operation_type IN ('OUT', 'PREPARE_CONSUMED') THEN ci.layer",
            "   ELSE fr.from_layer",
            " END AS fromLayer,",
            " CASE",
